...
public class ProfileGF<T extends Matter> {
	...
	{
		this.gf = gf;
		updateBackWardEdgeMap();
	}
	
	private HashMap<T, HashMap<Integer, T>> backwardEdgeMap;
	private HashMap<T, ScoreDist> bwdTable = null;
	private float sizeDictionary = 0;
	private Profile<T> profile = null;
	
	private void updateBackWardEdgeMap(){
		backwardEdgeMap = new HashMap<T, HashMap<Integer, T>>();
		
		for(T node : gf.getGraph().getIntermediateNodeList()){
			for(Edge<T> edge : gf.getGraph().getEdges(node)){
				T prevNode = edge.getPrevNode();
				int index = edge.getEdgeIndex();
				
				if(!backwardEdgeMap.containsKey(prevNode))
					backwardEdgeMap.put(prevNode, new HashMap<Integer, T>());
				
				backwardEdgeMap.get(prevNode).put(index, node);
			}
		}
		
		for(T node : gf.getGraph().getSinkList()){
			for(Edge<T> edge : gf.getGraph().getEdges(node)){
				T prevNode = edge.getPrevNode();
				int index = edge.getEdgeIndex();
	...
	private void setBackwardNodes(T curNode, HashMap<T, ScoreDist> bwdTable)
	{
		...
		int curScore = pointer.getNodeScore();

		int bits = 0;
		ScoreDist[] prevBwdDists = new ScoreDist[gf.getGraph().getAASet().size()];
		int[] edgeScores = new int[prevBwdDists.length];
		for(int score=curBwdDist.getMaxScore()-1; score>=curBwdDist.getMinScore(); score--)
		{
			...
			for(int aaIndex : pointer.getBacktrackAAIndexList(score))
			{
				if((bits & (1 << aaIndex)) == 0){
					...
					T prevNode = null;
					int es = 0;
					for(Edge<T> e : gf.getGraph().getEdges(curNode)){
						if(e.getEdgeIndex() == aaIndex){
					...
					if(prevNode != null){
						...
						edgeScores[aaIndex] = es;
					}
						//backwardEdgeMap.get(curNode).get(aaIndex);
					//gf.getGraph().getPreviousNode(curNode, gf.getAASet().getAminoAcid(aaIndex));
			...
			//	T prevNode = gf.getGraph().getPreviousNode(curNode, gf.getAASet().getAminoAcid(aaIndex));
				...
				if(prevBwdDist != null)
					prevBwdDist.addNumber(score-curScore - edgeScores[aaIndex], numberRecs);// TODO curScore + edge score
		...
		for(int score=curBwdDist.getMaxScore()-1; score>=curBwdDist.getMinScore(); score--)
		{
			...
			for(Edge<T> edge : gf.getGraph().getEdges(curNode))
			{
				...
				ScoreDist prevBwdDist = prevBwdDists[aaIndex];
				if(prevBwdDist != null){
					System.out.println(curBwdDist.getMaxScore() + "\t" + score + "\t" + curNodeScore);
			...
			for(int aaIndex : pointer.getBacktrackAAIndexList(score))
			{
				if((bits & (1 << aaIndex)) == 0){
					bits |= (1 << aaIndex);
					//System.out.println(curNode + "\t" + backwardEdgeMap.get(curNode));
					T prevNode = backwardEdgeMap.get(curNode).get(aaIndex);
						
					//	(curNode, gf.getGraph().getAASet().getAminoAcid(aaIndex));
					prevBwdDists[aaIndex] =  bwdTable.get(prevNode);
				}
			//	T prevNode = backwardEdgeMap.get(curNode).get(aaIndex);//gf.getGraph().getPreviousNode(curNode, gf.getGraph().getAASet().getAminoAcid(aaIndex));
				ScoreDist prevBwdDist = prevBwdDists[aaIndex];
				if(prevBwdDist != null)
					prevBwdDist.addNumber(score-curNodeScore, numberRecs);
			}
