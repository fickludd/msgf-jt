...
public class ModMatchObject extends MatchObject {
  ...
  public Peptide getPeptide() {
	  if(this.getPepideWithExactModifications() == null)
		  return new Peptide(getMatchAsString());
	  else 
		  return this.getPepideWithExactModifications();
  }
  ...
  public int getModStart(){ return modStart; }
  
  public int getModEnd(){ return modEnd; }
  ...
  public static String getSummaryHeader() {
     ...
     * 8. Score
     * 9. Protein name
     * 10. Start position in the protein
     * 11. End position in the protein
     * 12. Offset (Experimental Mass - Theoretical Mass)
     * 13+ Additional fields depending on the type of match object
     */
    sb.append("SpectrumFile\t");
    sb.append("Scan#\t");
    sb.append("ActMethod\t");
  ...
  public String getSummaryLine(String filename, 
    ...
    sb.append(this.getLeftFlankingAA());
    sb.append(".");
    //sb.append(getPeptide());
    for(int i=0; i<getPeptide().toString().length();i++){
    	char a = getPeptide().toString().charAt(i);
    	if(Character.isLowerCase(a)){
    		sb.append(Character.toUpperCase(a)+ (offset>0? "+" : "") + String.format("%.3f", offset));
    	}else
    		sb.append(a);
    }
    
    sb.append(".");
    sb.append(this.getRightFlankingAA());
    sb.append("\t");
    
    // 7. Probability
    sb.append(String.format("%.4e", this.getProb()));
    sb.append("\t");
