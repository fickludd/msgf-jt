...
public abstract class MatchObject {
  ...
  // stores the score of this match. 
  ...
  // added by kyowon for peptide matches with variable modifications
  private Peptide pepideWithExactModifications = null;
  /**
  ...
  public boolean isCleaved(Enzyme enzyme){
	  if(enzyme == null) return true;
	  
	  boolean isCleaved;
		  
	  if(enzyme.isCTerm()){
		  isCleaved = this.getRightFlankingAA() == '*' ||enzyme.isCleaved(this.getPeptide());		  
		  isCleaved &= this.getLeftFlankingAA() == '*' || enzyme.isCleavable(this.getLeftFlankingAA());
	  }else{
		  isCleaved = this.getLeftFlankingAA() == '*' ||enzyme.isCleaved(this.getPeptide());		  
		  isCleaved &= this.getRightFlankingAA() == '*' || enzyme.isCleavable(this.getRightFlankingAA());
	  }
  ...
  public static String array2string(ArrayList<Integer> a) {
	...
	public void setPepideWithExactModifications(Peptide pepideWithExactModification) {
		this.pepideWithExactModifications = pepideWithExactModification;
	}
	
	public Peptide getPepideWithExactModifications() {
		return pepideWithExactModifications;
	}
}
