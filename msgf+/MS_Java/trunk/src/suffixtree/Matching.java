...
public class Matching {
  ...
  static {
    ...
    while (it1.hasNext()) {
      ...
      while (it2.hasNext()) {
        ...
        if(mutant.isModified()) continue;
      //  if (mutant.getResidueStr()!=aa.getResidueStr()) {
          int massDiff = aa.getNominalMass() - mutant.getNominalMass();

          if (!mutTableList.get(aa.getResidue()).containsKey(massDiff)) {
            mutTableList.get(aa.getResidue()).put(massDiff, new ArrayList<Character>());
          }
          mutTableList.get(aa.getResidue()).get(massDiff).add(mutant.getResidue()); 
      //  }
      }
      // add the deletion possibility 
      if (!mutTableList.get(aa.getResidue()).containsKey(aa.getNominalMass())) {
        mutTableList.get(aa.getResidue()).put(aa.getNominalMass(), new ArrayList<Character>());
      } 
      mutTableList.get(aa.getResidue()).get(aa.getNominalMass()).add(Constants.EMPTY_AA); 
    }
  ...
  public static void matchDbWithModification(MassSequence db, 
    ...
    int prevMass = dbb[Math.min(dbbIndex,dbb.length-1)];
    ...
    for (int endPosition=dbbIndex; endPosition<dbb.length; endPosition++) {
      ...
      if (delta > Math.min(prevMass-1, -Constants.MIN_MOD)) break;//if (delta > -Constants.MIN_MOD) break;
      // do not allow mutations that do not change the mass
    //  if (delta == 0) continue;
     
      mods.add(new Modification(endPosition+1, -delta, dbStart+dbbIndex, dbStart+endPosition+1));
      
      /*for(long i = dbStart+dbbIndex; i<dbStart+endPosition+1; i++){
    	  int tmpMass = dbb[Math.min((int)(i-dbStart),dbb.length-1)];
    	  if (delta > Math.min(tmpMass-1, -Constants.MIN_MOD)) continue;
    		  
    	  mods.add(new Modification(endPosition+1, -delta, i, dbStart+endPosition+1));
      }*/
  ...
  public static void matchDbWithModificationR(MassSequence db, 
    ...
    for (int endPosition=dbbIndex; endPosition<dbb.length; endPosition++) {
      ...
      // no deletion can reconcile the masses difference
      if (delta > Math.min(currentMass-1, -Constants.MIN_MOD)) break;
     ...
     // if (delta == 0) continue;
      ...
      /*
      for(long i = dbStart-endPosition; i<dbStart-dbbIndex+1; i++){
    	  int tmpMass = dbb[Math.min((int)(-i+dbStart),dbb.length-1)];
    	  if (delta > Math.min(tmpMass-1, -Constants.MIN_MOD)) continue;
    	
    	  mods.add(new Modification(endPosition+1, -delta, i, dbStart-dbbIndex+1));
      }
     */
    }
  }
