...
import uninovoOld.ScoringDataIterator;
...
public class ModificationMatching {
  ...
  private static void matchAndScoreForUniNovo(String outFile,
		  String scoringFile,
          GappedPeptideResults gpr, 
          ProteinFastaSequence pfs,
          AminoAcidSet aaSet,
	...
	String matchDir = outFile + "ModMatchesDir";
	int matchedQueries = blindMatching(gpr, pfs, matchDir);
	
	// create a new Scoring Parameter iterator per call
	Iterator<ScoringData> sd = new ScoringDataIterator(scoringFile, aaSet, enzyme);
	Scoring.scoreModdedMatches(pfs, gpr, matchDir, sd, matchedQueries, aaSet, out);
}
  ...
  public static void run(Parameters params) {
    ...
    Iterator<GappedPeptideResults> it = new MSGDResultFileParser(grcFile, queryCount, params.delta()).iterator();
  ...
  public static void runForUniNovo(String outFile, String grcFile, 
		  String scoringFile,
		  String fasta, 
		  AminoAcidSet aaSet,
		  Enzyme enzyme) {
	   ...
	   Constants.AA = aaSet;
	    ...
	    // calculate how many results to parse at a time based on the total free memory
	    int queryCount = (int)(Runtime.getRuntime().maxMemory() * 0.0001);
	    ...
	    // the GRC file iterator
	    Iterator<GappedPeptideResults> it = new MSGDResultFileParser(grcFile, queryCount, 0).iterator();
	    ...
	    while (it.hasNext()) {
	      ...
	      if (new File(fasta).isDirectory()) {
	        ProteinFastaSequences db = new ProteinFastaSequences(fasta, sequences.Constants.AMINO_ACIDS_18, aaSet, false); 
	        Iterator<ProteinFastaSequence> pfsIt = db.getSequenceIterator();
	        while (pfsIt.hasNext()) {
	        	matchAndScoreForUniNovo(outFile, scoringFile, gpr, pfsIt.next(), aaSet, enzyme, out);
	        }
	      }
	      else {
	        ProteinFastaSequence db = new ProteinFastaSequence(fasta, sequences.Constants.AMINO_ACIDS_18, aaSet);
	        matchAndScoreForUniNovo(outFile, scoringFile, gpr, db, aaSet, enzyme, out);
	      }
