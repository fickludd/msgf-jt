...
public class MutationMatching {
   ...
   * @param out
   */
  private static void matchAndScoreForAdaNovo (String outFile,
		  String scoringFile,
          GappedPeptideResults gpr, 
          ProteinFastaSequence pfs,
          AminoAcidSet aaSet,
    ...
    String matchDir = outFile + "MutMatchesDir";
    
    int matchedQueries = oneMutationMatching(gpr, pfs, matchDir);
    Iterator<ScoringData> sd = new ScoringDataIterator(scoringFile, aaSet, enzyme);
    // create a new Scoring Parameter iterator per call
    Scoring.scoreMutatedMatches(pfs, gpr, matchDir, sd, matchedQueries, aaSet, out);
  }
  ...
  public static void runForUniNovo(String outFile, String grcFile, 
	    ...
	    while (it.hasNext()) {
	      ...
	      if (new File(fasta).isDirectory()) {
	    	  ProteinFastaSequences db =  new ProteinFastaSequences(fasta, sequences.Constants.AMINO_ACIDS_18_X, aaSet, false);
	          Iterator<ProteinFastaSequence> pfsIt = db.getSequenceIterator();
	        while (pfsIt.hasNext()) {
	        	matchAndScoreForAdaNovo(outFile, scoringFile, gpr, pfsIt.next(), aaSet, enzyme, out);
	        }
	      }
	      else {
	    	 ProteinFastaSequence db =  new ProteinFastaSequence(fasta, sequences.Constants.AMINO_ACIDS_18_X, aaSet);
	         matchAndScoreForAdaNovo(outFile, scoringFile, gpr, db, aaSet, enzyme, out);
	      }
