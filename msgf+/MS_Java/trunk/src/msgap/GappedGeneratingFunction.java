...
public class GappedGeneratingFunction<T extends Matter> extends GeneratingFunction<T>{
	...
	public GappedGeneratingFunction(DeNovoGraph<T> graph, Parameters par){
		super(graph);
		this.enzyme(par.enzyme());
		
		/*
		if(par.allowNonEnzymaticCleavage()){
			GeneratingFunction<T> gf = new GeneratingFunction<T>(graph);
...
//		return getGraph().getNominalPeptideMass();
	...
	private void setCurNode(T curNode, ScoreDistFactory factory)
	{
		int curScore = super.getGraph().getNodeScore(curNode); 
		
		//System.out.println(curScore + "\t" + this.getFwdTable().get(curNode).getMaxScore());
...
//		for(T prevNode : getGraph().getPreviousNodes(curNode)){
		...
		int maxEdgeScore = Integer.MIN_VALUE;
		int minEdgeScore = Integer.MAX_VALUE;
		...
		for(int hubIndex=hubSet.size()-1; hubIndex>=0; hubIndex--){
			...
			for(Edge<T> e : hubConnectedEdges){
				if(e.getPrevNode().equals(hub)){
					hubConnected = true;
					maxEdgeScore = Math.max(maxEdgeScore, e.getEdgeScore());
					minEdgeScore = Math.min(minEdgeScore, e.getEdgeScore());
					//break;
				}
		...
		for(int hubIndex : connectingGapScoreRange.keySet()){
			...
			ScoreDist curGapDist = factory.getInstance(curScore + minEdgeScore + range[0], curScore + maxEdgeScore + range[1]);
			
			if(he != null){
				if(calcProb()) curGapDist.addProb(curScore + he.getEdgeScore(), he.getEdgeProbability());
				if(ModifiedAAinGap.isAnyAAModified()){
					
					for(AminoAcid aa : super.getGraph().getAASet())
						if(aa.getNominalMass() == curNode.getNominalMass()-hub.getNominalMass())
			...
			for(Edge<T> e : nonHubConnectedEdges){ // from all the hubs ~ connected white
				...
				if(calcProb())
					...
					curGapDist.addProbDist(prevGapDist, curScore + e.getEdgeScore(), e.getEdgeProbability());
			...
			//System.out.println(curNode + "\t" + curGapDist.getMaxScore());
		...
		//System.out.println(curNode+ "\t" +minEdgeScore + "\t" + maxEdgeScore);
		//System.out.println(this.getFwdTable().get(this.getGraph().getSource()).getMaxScore() + "\t" +this.getGraph().getSource());
		...
		//	System.out.println(curNode + "\t" + curDist.getMinScore() + "\t" + curDist.getMaxScore());
