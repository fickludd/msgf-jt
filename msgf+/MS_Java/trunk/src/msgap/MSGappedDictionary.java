...
public class MSGappedDictionary {
	static private boolean verbose = true;
	static private int numSpecLimit = Integer.MAX_VALUE;
	static private int minScanNum = -1;
	static private boolean test = true; // TODO make it false
	static private NominalMassFactory factory = null;
	...
	*/
	...
	static private boolean isQualifiedAfterPMCorrection(Spectrum s, Parameters par, boolean correct){
	   ...
	   // }
        ...
        for(float pm : parentMasses){
		  ...
		  AminoAcidGraph g =  new AminoAcidGraph(factory, s2.getParentMass(), ss);

	      gf = new GeneratingFunction<NominalMass>(g).enzyme(par.enzyme());
	...
	static private int writeGappedSpectralDictionary(String spectrumFileName, PrintWriter grcFile, PrintWriter scoringParaFile, int cumCount, Parameters par){
		...
		while(iterator.hasNext())
		{
			...
			AminoAcidGraph graph =  new AminoAcidGraph(factory, spec.getParentMass(), scoreSpec);

			GappedGeneratingFunction<NominalMass> gap = new GappedGeneratingFunction<NominalMass>(graph, par);
			...
			ScoringParameter scoringPara = new ScoringParameter(cumCount, spec, spectrumFileName, gap, originalParentMass, scoreSpec);
			...
			for(GappedReconstruction<NominalMass> gappedReconstruction : gap.generateGappedDictionary(Math.max(scoreThreshold, par.matchScore()))){
				...
				//System.out.println(gappedReconstruction.getGapMassRepresentation());
				...
				//System.out.println(isCorrect +" " + spec.getAnnotation().isGappedPeptideTrue(gappedReconstruction.getPrefixMassRepresentation()));
			}
	...
	public static void run(Parameters par, boolean ver){
		...
		initialize(par);		
		factory = new NominalMassFactory(par.aaSet(), par.enzyme(), 50);
