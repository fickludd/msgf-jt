...
public class SpectrumMatches {
  ...
  public float addMatches(ArrayList<MatchObject> matches) {
    ...
    for (MatchObject mo : matches) {
      ...
      if (!seen.containsKey(key)) {
    	...
    	Annotation pep = new Annotation(par.aaSet().getAminoAcid(mo.getLeftFlankingAA()), mo.getPeptide(),  par.aaSet().getAminoAcid(mo.getRightFlankingAA()));
		    
      //  Annotation pep = new Annotation(flankingStr, par.aaSet());
        int score = getScore(pep);
        //mo.setScore(score);
        float p = getProbability(pep, score);
        if (p!=0) {
          mo.setProb(p);
          mo.setScore(score);
          seen.put(key, mo);
   ...
   * @param pep peptide annotation       
   * @return the peptide score
   */
  public int getScore(Annotation pep){
	//  System.out.println(this.getScanNumber() + " " + pep );
     
    if(Math.abs(pep.getPeptide().getParentMass() - spar.getCorrectedParentMass()) > par.pmTolerance().getToleranceAsDa(spar.getCorrectedParentMass())
    		|| spar.getGraph().getPMNode().getNominalMass() != pep.getPeptide().getNominalMass()) {
    		
    	//System.out.println(scoredSpec);
    	
      return spar.getFinalDist().getMinScore();
    }
      
  //  if(!par.enzyme().isCleaved(pep))
    //    spar.getGraph().allowNonEnzymaticCleavage();
        
    return spar.getGraph().getScore(pep);
  }
   ...
   */
  public float getProbability(Annotation pep, int psmScore){
    if(psmScore == spar.getFinalDist().getMinScore()) return 1;
    
    return spar.getFinalDist().getSpectralProbability(psmScore);
  }
