...
public class SuffixArray {
	...
	public ArrayList<String> getAllMatchedStrings(String pattern, int lengthFlankingStr)
	{
		...
		for(int i=0; i<matchSet.getSize(); i++)
		{
			...
			String rightStr = sequence.getSubsequence(end, Math.min(end+lengthFlankingStr, sequence.getSize())); // bug fixed by kyowon
	 ...
	 * @return a list of matched strings.
	 */
	public ArrayList<String> getAllMatchedStrings(String pattern, int lengthNFlankingStr, int lengthCFlankingStr)
	{
		MatchSet matchSet = findAll(pattern);
		ArrayList<String> matches = new ArrayList<String>();
		for(int i=0; i<matchSet.getSize(); i++)
		{
			int start = matchSet.getStart(i);
			int end = matchSet.getEnd(i);
			String leftStr = sequence.getSubsequence(Math.max(0, start-lengthNFlankingStr), start);
			String rightStr = sequence.getSubsequence(end, Math.min(end+lengthCFlankingStr, sequence.getSize())); // bug fixed by kyowon
			matches.add(leftStr+"."+sequence.getSubsequence(start, end)+"."+rightStr);
		}
		return matches;
	}
