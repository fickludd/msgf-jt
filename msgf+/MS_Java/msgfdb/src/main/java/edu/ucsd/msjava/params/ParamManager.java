...
public class ParamManager {
	...
	{
		FileParameter specFileParam = new FileParameter("s", "SpectrumFile", "*.mzML, *.mzML.gz, *.mzXML, *.mgf, *.ms2, *.pkl or *_dta.txt");
		specFileParam.addFileFormat(SpecFileFormat.MZML);
		specFileParam.addFileFormat(SpecFileFormat.MZML_GZ);
...
//		outputParam.fileMustNotExist();
	...
	public void addMSGFPlusParams()
	{
		...
		FloatParameter intensityThreshold = new FloatParameter("intensityThreshold", "IntensityThreshold", "Remove MS peaks below this level, Default: 1");
		intensityThreshold.minValue(0.0f);
		intensityThreshold.defaultValue(1.0f);
		addParameter(intensityThreshold);
